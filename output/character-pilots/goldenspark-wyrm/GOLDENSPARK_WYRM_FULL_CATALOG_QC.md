...
- Runtime sheet location: `assets/characters/goldenspark-wyrm/<action>/`; runtime portrait: `assets/characters/goldenspark-wyrm/portrait/`
...
- Raw generations, rejected provenance, alpha sources, normalization reports, processor metadata and previews remain for review; selected PNGs move by tracked rename into the runtime root, preserving their reviewed bytes.
...
Goldenspark Wyrm's selected portrait and complete catalog are approved for `assets/**` promotion. Registry／manifest／persona／browser regression and all human launch gates remain owned by the integrating TASK_PACK; this QC record alone does not declare launch readiness.
