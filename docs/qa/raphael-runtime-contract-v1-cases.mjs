...
const localOnly = await shadow.compare(request({ id: "device", careProcessing: "device" }));
checks.push(result("SHADOW-CONSENT-BLOCK", localOnly.shadowAttempted === false && hostedCalls === 1));
...
function request({ text = "今天想安靜一下", id = "valid", careProcessing = "not_care" } = {}) {
  return {
    ...
    client: { productId: "nexus-link", clientVersion: "test", instanceId: "fixture", locale: "zh-TW" },
    actor: { companionId: "greyshade-cat", personaVersion: "test" },
    input: { text, source: "soul_talk", timestamp: 1_786_000_000_000 },
    context: { stateVersion: 7, scene: "moonlake", relationship: { bond: 10, trust: 9, defense: 2 }, signals: { energy: 7, mood: "calm" } },
