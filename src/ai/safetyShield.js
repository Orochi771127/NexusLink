...
const CRISIS_RESOLUTION_LOCATION = /(?:(?:已經|已经|剛剛|刚刚)(?:人在|身在|已在|到(?:了|達|达)?|抵達|抵达|待在)|(?:目前|現在|现在|此刻)(?:人)?(?:已經|已经)?(?:就在|已在|人在|身在|待在|到(?:了|達|达)?|抵達|抵达|在)|(?:我|我們|我们)(?:人)?(?:就)?在)[^。！？!?\n]{0,10}(?:急診|急诊|醫院|医院|診所|诊所|警察局|派出所|消防隊|消防队|安全(?:的)?地方|庇護所|庇护所|可信任(?:的人)?身邊|可信任(?:的人)?身边)/i;
const CRISIS_RESOLUTION_LOCATION_EN = /\b(?:i'?m|i\s+am|we'?re|we\s+are)\s+(?:now\s+)?(?:at|in|inside)\s+(?:the\s+|a\s+)?(?:er\b|emergency\s+(?:room|department)|hospital|clinic|urgent\s+care|police\s+station|fire\s+station|shelter|safe\s+place)/i;
// 責任接手名詞刻意收得比原版廣：原版只認「救護人員／醫護人員／警察／社工／
// 醫師／護理師／可信任的人／家人／朋友」，導致「救護車已經到了」「我媽媽已經
// 到了」這類最自然的說法無法解除，形成永久鎖死。
const CRISIS_RESPONDER_NOUN = "救護車|救护车|救護員|救护员|救護人員|救护人员|醫護人員|医护人员|醫護|医护|急救人員|急救人员|消防(?:員|员|隊|队)?|警消|警察|員警|民警|社工|醫師|医师|醫生|医生|護理師|护理师|護士|护士|心理師|心理师|危機(?:處理|处理)(?:人員|人员)?|可信任的人|家人|爸媽|爸妈|媽媽|妈妈|母親|母亲|爸爸|父親|父亲|哥哥|姊姊|姐姐|弟弟|妹妹|伴侶|伴侣|配偶|朋友|室友|鄰居|邻居|老師|老师|同事|paramedics?|ambulance|emts?|doctors?|nurses?|police|officers?|social\\s+worker|my\\s+(?:mom|mum|mother|dad|father|friend|partner|sister|brother|neighbou?r|roommate|family)";
const CRISIS_ARRIVED_MARKER = "(?:已經|已经)(?:到(?:了|場|场)?|來|来|抵達|抵达|在我身邊|在我身边|在我旁邊|在我旁边|陪著我|陪着我|接手|協助|协助|幫忙|帮忙|處理|处理)|到了|來了|来了|抵達了|抵达了|在我身邊|在我身边|在我旁邊|在我旁边|陪著我|陪着我|(?:正在|現在|现在)(?:接手|處理|处理|協助|协助|幫忙|帮忙|照顧我|照顾我)|has\\s+arrived|have\\s+arrived|(?:just\\s+)?arrived|is\\s+here|are\\s+here|is\\s+with\\s+me|are\\s+with\\s+me|took\\s+over|(?:is|are)\\s+taking\\s+over|(?:is|are)\\s+handling";
const CRISIS_RESOLUTION_RESPONDER = new RegExp(
  `(?:${CRISIS_RESPONDER_NOUN})[^。！？!?\\n]{0,16}?(?:${CRISIS_ARRIVED_MARKER})`,
  "i"
);
const CRISIS_RESOLUTION_CONTACT = /(?:已經|已经|剛剛|刚刚).{0,10}(?:聯絡|联系|打給|打给|打了|撥打|拨打|叫了|叫來|叫来).{0,10}(?:119|110|113|1925|911|999|112|急救|救護車|救护车|警察|醫師|医生|急診|急诊)/i;
const CRISIS_RESOLUTION_CONTACT_EN = /\b(?:already\s+|just\s+)?(?:called|phoned|contacted|rang|dialed|dialled)\s+(?:(?:911|999|112|119|110)\b|(?:the\s+)?(?:ambulance|police|paramedics|emergency\s+services|emergency|a\s+doctor|crisis\s+line|hotline))/i;
const CRISIS_RESOLUTION_SAFE_NOW = /(?:我)?(?:現在|现在|目前|此刻)?(?:已經|已经)?(?:安全了|在安全的地方|沒有立即危險|没有立即危险)|\b(?:i'?m|i\s+am)\s+safe\s+now\b/i;
const CRISIS_RESOLUTION_HAZARD_REMOVED = /(?:工具|刀|藥|药|藥物|药物|危險物|危险物).{0,14}(?:已經|已经)?(?:交給|交给|移開|移开|收走|拿走|放遠|放远|丟掉|丢掉|沖掉|冲掉|鎖起來|锁起来)|\b(?:the\s+)?(?:pills?|knife|blade|weapon|meds?)\b.{0,20}(?:(?:have|has|been|were|are)\s+)?(?:taken\s+away|removed|locked\s+up|thrown\s+(?:away|out)|flushed)/i;
// 明確、可被玩家看見的退出語。危機連續性一旦鎖上就沒有任何逾時或
// session 重置，若不提供可發現的退出語，任何未使用上述精確句型的玩家
// 都會永久失去心語功能。退出語本身仍是本機 terminal、零獎勵，
// 且下一句若再次觸發高風險會立即重新上鎖。
const CRISIS_RESOLUTION_EXPLICIT_EXIT = /(?:結束|结束|解除|離開|离开|退出)\s*(?:安全模式|守護模式|守护模式)|\b(?:end|exit|leave)\s+safe\s+mode\b/i;
...
export function hasExplicitCrisisResolution(inputText = "") {
  ...
  return CRISIS_RESOLUTION_EXPLICIT_EXIT.test(text)
    || CRISIS_RESOLUTION_LOCATION.test(text)
    || CRISIS_RESOLUTION_LOCATION_EN.test(text)
    || CRISIS_RESOLUTION_RESPONDER.test(text)
    || CRISIS_RESOLUTION_CONTACT.test(text)
    || CRISIS_RESOLUTION_CONTACT_EN.test(text)
