...
- **Companion Growth / evolution / Growth UI / Codex stage**：開工前必讀 `docs/design/COMPANION_GROWTH_CONTRACT_V1.md`。該檔是現行設計／驗收 SSOT；G1 session-only 質性 Heart Phase 已接入，但 G2+ per-companion 持久狀態／migration／正式階段與形態 swap 尚未實作。不得把全域 bond 0／25／70 reveal、R2 五階、Expedition crafting 或 `src/ai/evolution/**` 誤稱為正式夥伴養成。Per-companion state／migration 與正式形態資產仍須分開取得 GROUNDWORK 核准。
...
- `docs/design/COMPANION_GROWTH_CONTRACT_V1.md` — 心核夥伴養成、三階覺醒、證據、安全、migration 與未來對練的設計／驗收 SSOT（G1 session-only 已接入；G2+ 尚未實作）。
